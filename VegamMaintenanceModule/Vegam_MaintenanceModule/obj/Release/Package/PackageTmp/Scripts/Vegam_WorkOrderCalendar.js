...
jQuery.WorkOrderCalendarNamespace.PageTypeList = { DayView: 'D', OrderView: 'O', ResourceView: 'R' };
jQuery.WorkOrderCalendarNamespace.PageType = '';

function LoadCalendarBasicInfo(basicParam, servicePath, maintenanceBasePath, coreBasePath,pType) {
    ...
    jQuery.WorkOrderCalendarNamespace.PageType = pType;
...
function BindCalendarTabs(featureInfoList) {
    ...
    if (jQuery.WorkOrderCalendarNamespace.DayViewFeaturePath.length > 0 && jQuery.WorkOrderCalendarNamespace.PageType == jQuery.WorkOrderCalendarNamespace.PageTypeList.DayView) {
        LoadIFrame(document.getElementById("lnkDayView"));
    }
    else if (jQuery.WorkOrderCalendarNamespace.OrderViewFeaturePath.length > 0 && jQuery.WorkOrderCalendarNamespace.PageType == jQuery.WorkOrderCalendarNamespace.PageTypeList.OrderView) {
        LoadIFrame(document.getElementById("lnkOrderView"));
    }
    else if (jQuery.WorkOrderCalendarNamespace.ResourceViewFeaturePath.length > 0 && jQuery.WorkOrderCalendarNamespace.PageType == jQuery.WorkOrderCalendarNamespace.PageTypeList.ResourceView) {
...
function WorkOrderCalendarIFrameLoad(obj) {
    ...
    jQuery("iframe").contents().find("#headerNextButton headerPrevButton").click(function () {
        tableHeight();
    });
    jQuery("iframe").contents().find("#headerPrevButton").click(function () {
        tableHeight();
    });
    tableHeight();
