...
jQuery.MeasuringPointListNameSpace.UploaderPath = "";


function LoadMeasuringPointListPage(gridProperties, basePath, uploaderPath) {
    ...
    jQuery.MeasuringPointListNameSpace.UploaderPath = uploaderPath;
...
function BindFieldIDVariables() {
    ko.utils.arrayForEach(dynamicGridViewModel.GridFieldList(), function (columnInfo) {
        if (columnInfo.FieldIdentifier != undefined && columnInfo.FieldIdentifier.toLowerCase() == "hdnmeasuringpointid")
            ...
            jQuery.MeasuringPointListNameSpace.FieldIDInfo.UserIDFieldID = columnInfo.FieldID;  
    });
}
    ...
    if (rowInfo.IsGroupRow) {
        ...
        if (fieldIdentifier.toLowerCase() == "action") {
           ...
           //var editHref = jQuery.MeasuringPointListNameSpace.BasePath + "/Preventive/AddMeasuringPoint.aspx?id=" + jQuery.MeasuringPointListNameSpace.BasicParam.SiteID + "&mpid=" + measuringPointID;
            ...
            rowActionInfoHtml = "<a class='fa fa-edit big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' onclick='javascript:AddMeasuringPoints(" + measuringPointID + ");return false;' title='Edit'></a>";
                     
            if (jQuery.MeasuringPointListNameSpace.PagerData.PageAccessRights.toLowerCase() == "full_access") {
                rowActionInfoHtml += "<i class='fa fa-trash-o red big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' title='Delete' onclick='DeleteMeasuringPoint(" + measuringPointID + ")'></i>";
            }
            else {
...
function GetCustomFilters(gridContentFilter) {
    if (jQuery.DynamicGridNamespace.FeatureAdditionalInfo == 'PLANTUSER') {
        var userIDField = {};
        userIDField.FieldID = jQuery.MeasuringPointListNameSpace.FieldIDInfo.UserIDFieldID;
        userIDField.FilterValueList = [];
        userIDField.FilterValueList.push(jQuery.MeasuringPointListNameSpace.BasicParam.UserID);
...
function TimerMethodForDownloadTemplate(fileName) {
            ...
            success: function (json) {
                if (json.d != null) {
                    if (json.d) {
                        ...
                        window.location = jQuery.MeasuringPointListNameSpace.BasePath + "/HandlerFiles/DownloadHandler.ashx?MeasuringPointTemplateFileName=" + fileName;
                        jQuery("#spnProgressExcel").addClass('hide');
                        EnableUploadControls();
                    }
                }
    ...
    errorTimer = setInterval(function () {
        jQuery.ajax({
            ...
            success: function (json) {
                if (json.d) {
                    jQuery.get(jQuery.MeasuringPointListNameSpace.BasePath + '/HandlerFiles/DownloadHandler.ashx?MeasuringPointTemplateFileName=' + errorTxtFile, function (data) {
                        clearInterval(successTimer);
                        clearInterval(errorTimer);
...
function UploadAllImages(fileName) {
    ...
    jQuery.ajax({
        type: "POST",
        url: jQuery.MeasuringPointListNameSpace.PagerData.ServicePath + "/Vegam_MaintenanceService.asmx/CheckDirectoryExist",
        ...
        success: function (json) {
            if (json.d) {
                jQuery.ajax({
                    url: jQuery.MeasuringPointListNameSpace.UploaderPath,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    responseType: "json",
                    data: { 'uid': jQuery.MeasuringPointListNameSpace.BasicParam.UserID, 'sid': jQuery.MeasuringPointListNameSpace.BasicParam.SiteID, 'measuringPointZip': 'true', fileName: imageFileName },
                    success: function (json) {
                        if (!json) {
...
function AddMeasuringPoints(measuringPointID) {
    if (measuringPointID == undefined)
        measuringPointID = 0;
    else
        SaveDynamicGridFilterValues();

    var queryString = "?id=" + jQuery.MeasuringPointListNameSpace.BasicParam.SiteID + "&mpid=" + measuringPointID;
