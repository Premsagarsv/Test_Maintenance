...
function BindFieldInfo(objectFieldInfo) {
    ...
    ko.utils.arrayForEach(objectFieldInfo.FieldInfoList, function (fieldInfo) {
        ...
        if (!jQuery.DynamicGridNamespace.DisableFilter && fieldInfo.FilterControlRenderType != jQuery.DynamicGridNamespace.FilterControlRenderType.None) {
            ...
            if (fieldInfo.IsFixedFilter == true || fieldInfo.IsUserViewFilterField == true) {
                RenderFilterControl(fieldInfo.FieldID, fieldInfo.DisplayName, fieldInfo.FilterControlRenderType, fieldInfo.FieldIdentifier, fieldInfo.FilterDefaultValueList, true, fieldInfo.FilterControlDataType);
            }
        }
...
function LoadDynamicGridContent(pagerData) {
    ...
    if (jQuery.DynamicGridNamespace.IsSearchClicked == false) {
        ...
        var tblBodyStartPos = jQuery("#tbDynamicGrid").offset().top;
        tblMaxHeight = totPageHeight - tblBodyStartPos - 82;
        if (jQuery(window).width() < 768)
            tblMaxHeight = 500;
        jQuery("#tbDynamicGrid").css("max-height", tblMaxHeight);
        //grid size for pages with pager.
        if (pagerData.PageIndex == 0) {
            if (enablePaging && Math.floor(tblMaxHeight / 37) > 2) {
                gridContentFilter.PageSize = Math.floor(tblMaxHeight / 37);
                pagerData.pageSize = Math.floor(tblMaxHeight / 37);
            }
...
function ChangeFilterSelection(filterInfo) { 
    if (filterInfo.IsSelected() == false) {
        ...
        RenderFilterControl(filterInfo.FieldID, filterInfo.DisplayName, filterInfo.FilterControlRenderType, filterInfo.FieldIdentifier, filterInfo.FilterDefaultValueList, false, filterInfo.FilterControlDataType);
    }
}
...
function RenderFilterControl(fieldID, fieldName, filterControlRenderType, customDataRenderType, filterDefaultValueList, isFixedFilter,filterControlDataType) {
    var controlString = "";
    var startValueCtlID = "";
    var endValueCtlID = "";
    ...
    if (filterDefaultValueList == undefined || filterDefaultValueList == null) {
        ...
        if (defaultValueList.length > 0) txtFilterDefaultValue = defaultValueList[0];
    }
    ...
    if (filterControlRenderType == jQuery.DynamicGridNamespace.FilterControlRenderType.Text) {
        ...
        if (filterControlRenderType == jQuery.DynamicGridNamespace.FilterControlRenderType.DateRange && (txtFilterDefaultValue == "currentdate" || txtFilterDefaultValue == "currentmonth" || txtFilterDefaultValue == "lasttwomonths" || txtFilterDefaultValue=="lastthreemonths")) {
        ...
        var enumTypeItemList = GetCustomFilterValues(customDataRenderType, filterControlRenderType);
...
function GetCustomFilterValues(customType, filterControlRenderType) {
    var enumMemberList = [];
            ...
            if (filterControlDivID != undefined) {
                ...
                if (fieldInfo.FilterDefaultValueList != undefined) {
                    ...
                    if (defaultValueList.length > 0) {
                        ...
                        else if (filterType == "daterange" && (defaultValueList[0] == "currentdate" || defaultValueList[0] == "currentmonth") || (defaultValueList[0] == "lasttwomonths")) {
