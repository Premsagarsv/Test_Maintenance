...
jQuery.EquipmentModelNamespace.MasterDataType = { "Manufacturer": 'M', "EquipmentType": 'T', "EquipmentClass": 'C', "MeasuringPointCategory": 'P', "MPSelectionGroupCode": 'G' };
...
function EquipmentModelLoadInfo(basicParam, maintTypePagerData, basePath, webServicePath, imagePath, uploaderPath, uploadImageAccess, modelID, equipmentModelImgPath, hasFullAccess) {
...
function LoadEquipmentModelList() {
    ...
    EquipmentFilterInfo.PageSize = 0;
    EquipmentFilterInfo.PageIndex = 0;
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            if (data.d != undefined) {
                var equipmentModelInfoList = data.d;
...
function LoadMasterDropDownList(isPageLoad, masterDataType) {
    ...
    if (isPageLoad || masterDataType == jQuery.EquipmentModelNamespace.MasterDataType.Manufacturer)
        masterDataDropDownListFilterInfo.MasterDataTypeList.push(jQuery.EquipmentModelNamespace.MasterDataType.Manufacturer.charCodeAt());
    if (isPageLoad || masterDataType == jQuery.EquipmentModelNamespace.MasterDataType.EquipmentType)
        masterDataDropDownListFilterInfo.MasterDataTypeList.push(jQuery.EquipmentModelNamespace.MasterDataType.EquipmentType.charCodeAt());
    if (isPageLoad || masterDataType == jQuery.EquipmentModelNamespace.MasterDataType.EquipmentClass)
        masterDataDropDownListFilterInfo.MasterDataTypeList.push(jQuery.EquipmentModelNamespace.MasterDataType.EquipmentClass.charCodeAt());
...
function AddUpdateEquipmentModel() {
    ...
    if (equipmentDetailsInfo.ClassID === "-1") {
        errorMsg += languageResource.resMsg_PleaseSelectClass + "</br>";
        isValid = false;
    }
...
function LoadMaintTypesInfo(pagerData) {
    ...
    masterDataFilterInfo.MasterDataType = jQuery.EquipmentModelNamespace.SelectedMasterDataType.charCodeAt();
...
function InsertOrUpdateMaintType(maintTypeID) {
    ...
    if (jQuery.trim(jQuery("#txtNewMaintType").val()).length == 0) {
        ...
        masterDataInfo.MasterDataType = jQuery.EquipmentModelNamespace.SelectedMasterDataType.charCodeAt();
