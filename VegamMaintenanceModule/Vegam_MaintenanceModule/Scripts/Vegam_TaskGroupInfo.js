...
jQuery.TaskGroupInfoNameSpace.MasterDataType = { "Manufacturer": "MANUFACTURER", "EquipmentType": "TYPE", "EquipmentClass": "CLASS", "MeasuringPointCategory": "MP_CATEGORY", "MPSelectionGroupCode": "GROUPCODE", "TaskGroupType": "TASKGRP_TYPE" };
...
var taskGroupViewModel = {
    ...
    HasPPEChanged: ko.observable(false),
    HasToolsChanged: ko.observable(false),
    HasDocumentChanged: ko.observable(false),
    HasParameterChanged: ko.observable(false),
    HasSparePartChanged: ko.observable(false),

    isMaintShowAll: ko.observable(false),
...
var newSparePartRow = function () {
    return {
        MaterialNumber: ko.observable(''),
        MaterialDesc: ko.observable(''),
...
function LoadTaskGroupInfo(maintTypePagerData, basicParam, basePath, webservicePath, uploaderPath, imagePath, taskGroupIdentifier, versionNumber, loadControlID, accessRights, taskDocumentPath, isUserHasApprovePermission, maintScheduledId, isMaintShowAll) {
    ...
    if (isMaintShowAll.toLowerCase() == "true") {
        taskGroupViewModel.isMaintShowAll(true);
...
function LoadMasterDataDropDown() {
    ...
    masterDataDropDownListFilterInfo.MasterDataTypeList.push(jQuery.TaskGroupInfoNameSpace.MasterDataType.TaskGroupType);
    masterDataDropDownListFilterInfo.MasterDataTypeList.push(jQuery.TaskGroupInfoNameSpace.MasterDataType.MPSelectionGroupCode);
...
function LoadSparePartMaterialList() {
    ...
    jQuery.ajax({
        type: "POST",
        url: jQuery.TaskGroupInfoNameSpace.ServicePath + "/Vegam_MaintenanceService.asmx/GetSparePartMaterialDetails",
        ...
        success: function (data) {
            if (data.d !== null) {
                ...
                jQuery.each(data.d, function (index, value) {
                    ...
                    taskGroupViewModel.SparePartMaterialList.push(material);
                    material = {};
                });
            } else {
                ShowMessage(languageResource.resMsg_FailedToLoadDropdown, "error");
        ...
        error: function (request, error) {
            ...
            ShowMessage(languageResource.resMsg_Error + errorMessage, "error");
        }
    });
}
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            if (data.d != undefined && data.d != null) {
                ...
                jQuery("#divFailedTaskGroupLoad").modal("show");
            }
        },
        beforeSend: function () {
            taskGroupViewModel.IsProgressing(true);
                        ...
                        default:
                            {
                                if (jQuery.TaskGroupInfoNameSpace.TaskGroupVersion === 0) {
                                    ...
                                    var queryString = "?id=" + jQuery.TaskGroupInfoNameSpace.BasicParam.SiteID + "&gid=" + taskGroupIdentifier.IdentifierID + "&v=" + taskGroupIdentifier.VersionNumber;
...
function BindTaskInfo(taskID) {
    ...
    taskGroupViewModel.HasPPEChanged(false);
    taskGroupViewModel.HasToolsChanged(false);
    taskGroupViewModel.HasDocumentChanged(false);
    taskGroupViewModel.HasParameterChanged(false);
    taskGroupViewModel.HasSparePartChanged(false);
        ...
        success: function (data) {
            if (data.d != undefined && data.d != null) {
                ...
                jQuery.each(taskInfoList, function (index, taskInfo) {
                    ...
                    //SparePart list
                    taskViewModelObj.SparePartInfoList.removeAll();
                    if (taskInfo.SparePartInfoList.length > 0) {
                        jQuery.each(taskInfo.SparePartInfoList, function (index, obj) {
                            var newSparePartObj = new newSparePartRow();
                            newSparePartObj.MaterialNumber(obj.MaterialNumber);
                            newSparePartObj.MaterialDesc(obj.MaterialDesc);
                            newSparePartObj.Quantity(obj.Quantity);
                            newSparePartObj.UOM(obj.UOM);
                            newSparePartObj.DisplayName(obj.MaterialNumber + ' - ' + obj.MaterialDesc);
                            newSparePartObj.isDeleteEnabled(true);
                            taskViewModelObj.SparePartInfoList.push(newSparePartObj);
                        });
                    }
                    else {
                        var newSparePartObj = new newSparePartRow();
                    ...
                    jQuery("#divSparePart_" + index + " select").each(function (i, ctrl) {
                        jQuery(ctrl).select2({ width: '100%' });
                        if (taskInfo.SparePartInfoList.length > 0 && taskInfo.SparePartInfoList[i] != undefined) {
                            jQuery(ctrl).val(taskInfo.SparePartInfoList[i].MaterialNumber);
...
function SaveTaskInformation() {
    ...
    taskInfo.EstimatedTime = taskInfo.EstimatedTime == "" ? 0 : taskInfo.EstimatedTime;
    ...
    var isSparePartValid = true;
    var sparePartInfoList = [];
    ko.utils.arrayForEach(SelectedTaskInfoViewModel.SparePartInfoList(), function (item, index) {
        var SparePartInfo = {};
        ...
        if (SparePartInfo.MaterialNumber != "" && SparePartInfo.Quantity == "")
            isSparePartValid = false;
        else if ((SparePartInfo.MaterialNumber != "" && SparePartInfo.Quantity != ""))
            sparePartInfoList.push(SparePartInfo);
    });

    taskInfo.SparePartInfoList = sparePartInfoList;
    ...
    if (taskInfo.Description === "") {
        errorMsg += languageResource.resMsg_PleaseEnterInstructions + "</br>";
        isValid = false;
    }

    if (!isSparePartValid) {
        errorMsg += languageResource.resMsg_PleaseEnterQuantity + "</br>";
        isValid = false;
    }
                        ...
                        default:
                            {
                                ...
                                if (jQuery.TaskGroupInfoNameSpace.SelectedTaskID == 0) {
                                    ...
                                    taskModel.TaskID = taskGroupIdentifier.SelectedTaskID;
...
function RemovePPIImage(instructionID) {
    ...
    ko.utils.arrayForEach(SelectedTaskInfoViewModel.TaskPPEList(), function (item, index) {
        if (item.InstructionID === instructionID) {
            ...
            taskGroupViewModel.HasPPEChanged(true);
        }
    });
...
function RemoveToolsImage(instructionID) {
    ...
    ko.utils.arrayForEach(SelectedTaskInfoViewModel.TaskToolsList(), function (item, index) {
        if (item.InstructionID === instructionID) {
            ...
            taskGroupViewModel.HasToolsChanged(true);
        }
    });
...
function AddMoreParameter() {
    ...
    if (SelectedTaskInfoViewModel.ParameterInfoList().length > 0) {
        ...
        if (isParameterEmpty) {
            ...
            taskGroupViewModel.HasParameterChanged(true);
        }
...
//region Spare Parts info method 
function OnSparePartChange(data, event) {
    var selectedMaterial = jQuery(event.target).val();
    if (selectedMaterial != "") {
        var SelectedTaskInfoViewModel = taskGroupViewModel.TaskInfoList()[0];
        var itemExists = ko.utils.arrayFirst(SelectedTaskInfoViewModel.SparePartInfoList(), function (item) {
            return item.MaterialNumber() == selectedMaterial;
        });
        if (itemExists && taskGroupViewModel.EnableEdit()) {
            ShowErrorMessage(languageResource.resMsg_MaterialIsAlreadySelected, true);
            jQuery(event.target).val("");
            jQuery(event.target).trigger("change");
        }
        else {
            var selectedItem = ko.utils.arrayFirst(taskGroupViewModel.SparePartMaterialList(), function (item) { return item.MaterialNumber == selectedMaterial; });
...
function AddMoreSparePartRow() {
    ...
    var SelectedTaskInfoViewModel = taskGroupViewModel.TaskInfoList()[0];
    var newSparePartObj = new newSparePartRow();
    if (SelectedTaskInfoViewModel.SparePartInfoList().length > 0) {
        var isParameterEmpty = false;
        ko.utils.arrayForEach(SelectedTaskInfoViewModel.SparePartInfoList(), function (item) {
            var enableDeleteIcon = true;
            if (item.MaterialNumber() === "") {
                isParameterEmpty = true;
                enableDeleteIcon = false;
                errorMsg += languageResource.resMsg_PleaseSelectMaterial;
            }
            else if (item.Quantity() == undefined || item.Quantity() === "") {
                isParameterEmpty = true;
                enableDeleteIcon = false;
                errorMsg += languageResource.resMsg_PleaseEnterQuantity;
            }
            item.isDeleteEnabled(enableDeleteIcon);
        });
        if (isParameterEmpty)
            ...
            SelectedTaskInfoViewModel.SparePartInfoList.push(newSparePartObj);
            taskGroupViewModel.HasSparePartChanged(true);
        }
    } else {
        SelectedTaskInfoViewModel.SparePartInfoList.push(newSparePartObj);
        taskGroupViewModel.HasSparePartChanged(false);
...
function DeleteSparePartInfo(sparePartInfo) {
    taskGroupViewModel.TaskGroupErrorMsg('');
    taskGroupViewModel.ModelErrorMsg('');
    jQuery("#alertMessage").removeClass("text-danger");
    jQuery("#alertMessage").html(languageResource.resMsg_AreYouSureYouWantToDeleteSpareParts);
    jQuery("#alertModal").dialog({
        zIndex: 1060,
        modal: true,
        buttons: [
            {
                ...
                click: function () {
                    jQuery("#alertModal").dialog("close");
                    var SelectedTaskInfoViewModel = taskGroupViewModel.TaskInfoList()[0];
                    SelectedTaskInfoViewModel.SparePartInfoList.remove(sparePartInfo);
                    taskGroupViewModel.HasSparePartChanged(true);
                    if (SelectedTaskInfoViewModel.SparePartInfoList().length == 0) {
                        var newSparePartObj = new newSparePartRow();
                        SelectedTaskInfoViewModel.SparePartInfoList.push(newSparePartObj);
...
//endregion Spare Parts info method 
        ...
        jQuery.ajaxFileUpload({
            ...
            success: function (data, status) {
                ...
                if (data.documentElement.innerText != "true") {
                    ...
                    taskGroupViewModel.HasDocumentChanged(true);
                }
            },
            error: function (request, error) {
                jQuery("#instructionFileUpload").val('');
...
function BindMasterDataInfo() {
    if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType == jQuery.TaskGroupInfoNameSpace.MasterDataType.TaskGroupType) {
        ...
        jQuery(".labelMasterDataType").removeClass("label-xxxlarge");
        jQuery(".labelMasterDataType").addClass("label-xxlarge");
    ...
    } else if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType == jQuery.TaskGroupInfoNameSpace.MasterDataType.MPSelectionGroupCode) {
        ...
        jQuery(".labelMasterDataType").removeClass("label-xxlarge");
        jQuery(".labelMasterDataType").addClass("label-xxxlarge");
        ...
        jQuery.TaskGroupInfoNameSpace.SelectedMasterDataMessage = languageResource.resMsg_SelectionGroup;
    }
}
...
function ClearMaintenanceTypeInfo() {
    ...
    if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataTypeID > 0) {
        LoadMasterDataDropDown();
        if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType != jQuery.TaskGroupInfoNameSpace.MasterDataType.MPSelectionGroupCode) {
...
function LoadMaintTypesInfo(pagerData) {
    ...
    masterDataFilterInfo.MasterDataType = jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType;
        ...
        success: function (json) {
            ...
            if (json != undefined && json.d != undefined) {
                if (json.d.MasterDataInfoList.length == 0) {
                    ...
                    if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType == jQuery.TaskGroupInfoNameSpace.MasterDataType.MPSelectionGroupCode)
                    { jQuery.each(json.d.MasterDataInfoList, function (i, masterDataInfo) {
                        masterDataInfo.Description =  masterDataInfo.SelectionGroupItemName;
                    });                    
                    }                
...
function ShowEditMaintTypes(maintMasterData) {
    ...
    jQuery("#spnAddMaintTypeError").html("");
    if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType == jQuery.TaskGroupInfoNameSpace.MasterDataType.MPSelectionGroupCode) {
        jQuery.each(maintMasterData.SelectionGroupItemName, function (index, itemValue) {
        ...
        autoheight('#txtMasterDescription').css({"height" : "50px"});
    }
...
function removeButton(elem) {
    if (jQuery.TaskGroupInfoNameSpace.MaintTypePagerData.PageAccessRights.toUpperCase() != "READ_ONLY") {
        jQuery(elem).detach();
    }
}
...
function DeleteMaintTypeInfo(maintTypeID) {
    ...
    masterDataInfo.MasterDataType = jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType;
    jQuery.ajax({
        type: "POST",
...
function InsertOrUpdateMaintType(maintTypeID) {
    ...
    if (jQuery.trim(jQuery("#txtNewMaintType").val()).length == 0) {
        errorMessage = errorMessage + languageResource.resMsg_EnterMaintType.replace("[XXX]", jQuery.TaskGroupInfoNameSpace.SelectedMasterDataMessage.toLowerCase().toString()) + "<br/>";
        isValid = false;
    }
    if (jQuery.TaskGroupInfoNameSpace.SelectedMasterDataType == jQuery.TaskGroupInfoNameSpace.MasterDataType.MPSelectionGroupCode) {
        jQuery("#divAddedItems button").each(function (i) {
