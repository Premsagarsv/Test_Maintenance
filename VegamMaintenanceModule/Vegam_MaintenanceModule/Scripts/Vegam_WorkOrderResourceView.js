...
function ShowNextPreview(dateText) {
    jQuery.WorkOrderResourceViewCalendarNamespace.CurrentDate = dateText;
    jQuery("#viewMonthCalendar .ui-datepicker-calendar").hide();
    jQuery('#viewMonthCalendar').datepicker('setDate', jQuery.WorkOrderResourceViewCalendarNamespace.CurrentDate);
    LoadResourceViewTimeline(jQuery.WorkOrderResourceViewCalendarNamespace.PagerData);
}
...
jQuery(document).ready(function () {
    ...
    jQuery(document).on('click', '.ui-datepicker-next', function () {
        if (jQuery.WorkOrderResourceViewCalendarNamespace.IsSearch == true) {
            ...
            var selectedD = new Date(jQuery.WorkOrderResourceViewCalendarNamespace.CurrentDate.toString().replace(/(\d\d\d\d)(\d\d)(\d\d)/g, '$2/$3/$1'));
            selectedD.setMonth(selectedD.getMonth() + 1);
            ShowNextPreview(selectedD.format('yyyyMMdd'));          
        }
    });
