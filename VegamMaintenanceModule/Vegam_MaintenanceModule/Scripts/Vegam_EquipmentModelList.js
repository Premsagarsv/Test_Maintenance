...
function CustomGridContentBind(rowInfo, rowIndex, columnInfo) {
    ...
    if (rowInfo.IsGroupRow) {
        ...
        if (fieldIdentifier.toLowerCase() == "action") {
            ...
            rowActionInfoHtml = "<a class='fa fa-edit big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' onclick='javascript:AddNewEquipmentModel(" + equipmentID + ");return false;' title='Edit'></a>";
           
            if (jQuery.EquipmentModelListNameSpace.PagerData.PageAccessRights.toLowerCase() == "full_access") {
                rowActionInfoHtml += "<i class='fa fa-trash-o red big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' onclick='DeleteEquipmentModel(" + equipmentID + ")' title='Delete'></i>";
            }
            else {
        ...
        if (fieldIdentifier.toLowerCase() == "imageshower") {
            ...
            content += "<span style='color:#2871a5' class='imageshower col-xs-12 p-a-0 cursor-pointer' onmouseenter='javascript:ShowImageOnClick(this);return false;' onmouseleave='javascript:HideImageOnClick(this);return false;'>" + rowInfo[fieldID];           
            content += "</span>";
        }
    }
    return content;
...
function ShowImageOnClick(rowCtl) {
    ...
    jQuery('.imageshowmod').removeClass('show').addClass('hide events-none');
    var x = jQuery(rowCtl).closest('td').offset().left;
    var yTranslate = jQuery(rowCtl).closest('td').offset().top;
    var xCompensate = jQuery(rowCtl).text().length;
    var posx;
    if (xCompensate < 10) {
        var wid = jQuery(rowCtl).closest('td').width();
        posx = x + wid / 2;
    } else {
        posx = x + (xCompensate * 10);
    }
...
function HideImageOnClick(rowCtl) {
    var rowInfo = ko.dataFor(jQuery(rowCtl).closest("tr")[0]);
    jQuery(rowCtl).css('transform', 'scale(1)');
    jQuery("#imageEquipmentModel").attr('src', '');
    jQuery('.imageshowmod').hide().removeClass('events-none');
}
...
function AddNewEquipmentModel(equipmentID) {
    if (equipmentID == undefined)
        equipmentID = 0;
    else
        SaveDynamicGridFilterValues();
    window.location = jQuery.EquipmentModelListNameSpace.BasePath + "/Preventive/AddEquipmentModel.aspx?id=" + jQuery.EquipmentModelListNameSpace.BasicParam.SiteID + "&eid=" + equipmentID;
}