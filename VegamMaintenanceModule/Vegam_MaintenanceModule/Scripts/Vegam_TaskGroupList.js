...
jQuery.TaskGroupListNamespace.UploaderPath = "";

function LoadTaskGroupListPage(gridProperties, basePath, uploaderPath) {
    ...
    jQuery.TaskGroupListNamespace.UploaderPath = uploaderPath;
    ...
    if (rowInfo.IsGroupRow) {
        ...
        if (fieldIdentifier.toLowerCase() == "action") {
            ...
            if (jQuery.TaskGroupListNamespace.PagerData.PageAccessRights.toLowerCase() == "full_access" || jQuery.TaskGroupListNamespace.PagerData.PageAccessRights.toLowerCase() == "edit_only") {
                rowActionInfoHtml = "<a class='fa fa-edit big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' title='Edit' onclick='javascript:AddNewTaskGroup(" + taskGroupIdentifierID + "," + taskVersionNum+");return false;'></a>";
            }
            else {
            ...
            if (jQuery.TaskGroupListNamespace.PagerData.PageAccessRights.toLowerCase() == "full_access") {
                rowActionInfoHtml += "<i class='fa fa-trash-o red big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' title='Delete' onclick='DeleteTaskGroup(" + taskGroupIdentifierID + "," + taskVersionNum + ",\"" + status + "\")'></i>";
            }
            else {
            ...
            if (taskApprovedVersionNum != 0 && taskApprovedVersionNum != taskVersionNum) {
                ...
                rowActionInfoHtml += "<a class='fa fa-eye blue big tiny-leftmargin tiny-rightmargin v-icon  cursor-pointer' title='" + languageResource.resMsg_ViewApprovedVersion + "' href='" + path + "'></a>";
            }
            else {
...
function DeleteTaskGroup(taskIdentifierID, versionNumber, taskGroupStatus) {
    jQuery("#alertMessage").removeClass("text-info");
    jQuery("#alertMessage").removeClass("text-danger");
    switch (jQuery.trim(taskGroupStatus).toUpperCase())
    {
        case 'A':
            jQuery("#alertMessage").html(languageResource.resMsg_AreYouSureYouWantToDeleteApprovedTaskGroup);
            break;
        case 'M':
            jQuery("#alertMessage").html(languageResource.resMsg_AreYouSureYouWantToDeleteModifiedTaskGroup);
            break;
        default:
        ...
        buttons: [
            {
                ...
                click: function () {
                    ...
                    DeleteConfirmedTaskGroup(taskIdentifierID, versionNumber);
                }
            },
            {
...
function DeleteConfirmedTaskGroup(taskIdentifierID, versionNumber) {
    var taskGroupFilterInfo = {};
    taskGroupFilterInfo.IdentifierID = taskIdentifierID;
    taskGroupFilterInfo.VersionNumber = versionNumber;

    jQuery.ajax({
        type: "POST",
        url: jQuery.TaskGroupListNamespace.WebServicePath + "/Vegam_MaintenanceService.asmx/DeleteTaskGroupInfo",
        data: JSON.stringify({ basicParam: jQuery.TaskGroupListNamespace.BasicParam, taskGroupfilterInfo: taskGroupFilterInfo }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
...
function DownLoadExcelTemplate() {
    ...
    jQuery.ajax({
        type: "POST",
        url: jQuery.TaskGroupListNamespace.PagerData.ServicePath + "/Vegam_MaintenanceService.asmx/DownloadSOPTemplate",
...
function TimerMethodsForDownloading(fileName) {
    var successTimer;
    var errorTimer;
            ...
            success: function (json) {
                if (json.d != null) {
                    if (json.d != false) {
                        ...
                        jQuery("#spnProgressExcel").addClass('hide');
                        EnableUploadControls();
                        window.location = jQuery.TaskGroupListNamespace.BasePath + "/HandlerFiles/DownLoadHandler.ashx?SOPFileName=" + fileName;
    ...
    errorTimer = setInterval(function () {
        jQuery.ajax({
            ...
            success: function (json) {
                if (json.d) {
                    jQuery.get(jQuery.TaskGroupListNamespace.BasePath + "/HandlerFiles/DownLoadHandler.ashx?SOPFileName=" + fileName, function (data) {
                        clearInterval(successTimer);
                        clearInterval(errorTimer);
...
function UploadTaskGroupInfo() {
    ...
    if (isValid) {
        jQuery.ajaxFileUpload({
            type: "POST",
            url: jQuery.TaskGroupListNamespace.BasePath + "/HandlerFiles/SOPUploadHandler.ashx" + '?sid=' + jQuery.TaskGroupListNamespace.PagerData.SiteID +
...
function TimerMethodForUpload(fileName) {
    ...
    var errorFileName = fileName + "_Error.txt";
    successTimer = setInterval(function () {
        jQuery.ajax({
            type: "POST",
            url: jQuery.TaskGroupListNamespace.PagerData.ServicePath + "/Vegam_MaintenanceService.asmx/CheckFileExist",
            ...
            success: function (json) {
                if (json.d) {
                    clearInterval(successTimer);
                    clearInterval(errorTimer);
                    UploadSOPAttachments(fileName);
                    EnableUploadControls();
                    window.location = jQuery.TaskGroupListNamespace.BasePath + "/HandlerFiles/DownLoadHandler.ashx?SOPLogFile=" + successTxtFile;
    ...
    errorTimer = setInterval(function () {
        jQuery.ajax({
            ...
            success: function (json) {
                if (json.d) {
                    jQuery.get(jQuery.TaskGroupListNamespace.BasePath + '/HandlerFiles/DownloadHandler.ashx?SOPLogFile=' + errorFileName, function (data) {
                        clearInterval(successTimer);
                        clearInterval(errorTimer);
                        EnableUploadControls();
                        ...
                        if (data.indexOf('Inserted :0') != -1 && data.indexOf('Updated :0') != -1 && data.indexOf('Failure :0') != -1) {
                            ...
                            window.location = jQuery.TaskGroupListNamespace.BasePath + "/HandlerFiles/DownloadHandler.ashx?SOPLogFile=" + errorFileName;
                        }
...
function UploadSOPAttachments(fileName) {
    //var fileName = "Temp" + fileName;
    jQuery.ajax({
        type: "POST",
        url: jQuery.TaskGroupListNamespace.PagerData.ServicePath + "/Vegam_MaintenanceService.asmx/CheckDirectoryExist",
        ...
        success: function (json) {
            if (json.d) {
                jQuery.ajax({
                    url: jQuery.TaskGroupListNamespace.UploaderPath,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    responseType: "json",
                    data: { 'uid': jQuery.TaskGroupListNamespace.BasicParam.UserID, 'sid': jQuery.TaskGroupListNamespace.BasicParam.SiteID, 'sopZip': 'true', fileName: fileName },
                    success: function (json) {
                         if (!json) {
                             jQuery("#lblUploadError").text(languageResource.resMsg_FailedToUploadSOPDocuments);
                         }
                     },
                     error: function (request, error) {
                         var errorMessage = languageResource.resMsg_Error + languageResource.resMsg_FailedToUploadSOPDocuments;
                         if (request.responseText != "") {
                         ...
                         jQuery("#lblUploadError").text(errorMessage);
                     }
                });


                //var dataToSend = "uid=" + jQuery.TaskGroupListNamespace.BasicParam.UserID + "&sid=" + jQuery.TaskGroupListNamespace.BasicParam.SiteID + "&sopZip=true&fileName=" + fileName;
                //jQuery.ajax({
                //    type: "POST",
                //    url: jQuery.TaskGroupListNamespace.UploaderPath+"?"+dataToSend,
                //            success: function (json) {
                //        if (!json) {
                //            jQuery("#lblUploadError").text(languageResource.resMsg_FailedToUploadSOPDocuments);
                //        }
        ...
        error: function (request, error) {
            if (request.responseText != "") {
                ...
                if (message != undefined && message != "" && message.Message != undefined && message.Message != "") {
                    ...
                    jQuery("#lblUploadError").html(languageResource.resMsg_FailedToUploadSOPDocuments);
                }
            }
            else {
                jQuery("#lblUploadError").html(languageResource.resMsg_FailedToUploadSOPDocuments);
...
function AddNewTaskGroup(taskGroupIdentifierID, taskVersionNum) {
    if (taskGroupIdentifierID == undefined) {
        taskGroupIdentifierID = 0;
        taskVersionNum = 0;
    }
    else {
